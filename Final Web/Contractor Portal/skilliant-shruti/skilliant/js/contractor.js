...
function getInitials(name) {
  ...
  if (parts.length >= 2) {
    // Always use the first letter of the first name and the first letter of the surname.
...
function initLabour(){
  ...
  const render=()=>{
    ...
    const totalCount=document.getElementById('labourTotalCount');
    const activeCount=document.getElementById('labourActiveCount');
    const inactiveCount=document.getElementById('labourInactiveCount');
    const leaveCount=document.getElementById('labourLeaveCount');
    if(totalCount) totalCount.textContent=all.length;
    if(activeCount) activeCount.textContent=all.filter(x=>x.status==='Active').length;
    if(inactiveCount) inactiveCount.textContent=all.filter(x=>x.status==='Inactive').length;
    if(leaveCount) leaveCount.textContent=all.filter(x=>x.status==='On Leave').length;
    ...
    body.innerHTML=list.map(x=>`<tr><td><div class="labour-cell"><div class="avatar-sm">${getInitials(x.name)}</div><div class="labour-name-box"><span class="labour-name">${x.name}</span><span class="labour-id-sub">${x.id}</span></div></div></td><td><span style="font-weight:600;">${x.skill}</span></td><td>${x.phone||'N/A'}</td><td>${x.city?`${x.city}, ${x.state||''}`:(x.address||'N/A')}</td><td>${getStatusBadgeHtml(x.status)}</td><td>${x.joiningDate||'N/A'}</td><td style="text-align:right"><div class="action-buttons" style="justify-content:flex-end"><a href="#labour-details?id=${encodeURIComponent(x.id)}" class="icon-action-btn" title="View Details">◉</a><a href="#edit-labour?id=${encodeURIComponent(x.id)}" class="icon-action-btn" title="Edit Labour">✎</a><button type="button" class="btn btn-secondary btn-sm labour-status-toggle" data-id="${x.id}" title="Change labour status">${x.status==='Active'?'Set Inactive':'Set Active'}</button><button type="button" class="icon-action-btn delete-btn" onclick="handleDeleteClick('${x.id}','${x.name.replace(/'/g,"\\'")}')">🗑</button></div></td></tr>`).join('');
    body.querySelectorAll('.labour-status-toggle').forEach(button=>{
      button.onclick=()=>{
        const id=button.dataset.id;
        const record=getLabourById(id);
        if(!record)return;
        const nextStatus=record.status==='Active'?'Inactive':'Active';
        const updated={...record,status:nextStatus};
        if(updateLabour(updated)){
          if(window.skilliantLogActivity) window.skilliantLogActivity('Labour status changed', `${updated.name} (${updated.id}) changed to ${nextStatus}.`);
          showToast(`${updated.name} is now ${nextStatus}.`,'success');
          render();
          if(typeof window.renderDashboardFinal==='function') window.renderDashboardFinal();
          if(typeof window.renderProjectsFinal==='function') window.renderProjectsFinal();
        }
      };
    });
  };
...
function initSettings(){
  ...
  if (resetPasswordBtn && !resetPasswordBtn.dataset.bound) {
    resetPasswordBtn.dataset.bound = '1';
    resetPasswordBtn.onclick = () => {
      ...
      localStorage.setItem('skilliant_password_v1', JSON.stringify({ password, updatedAt: new Date().toISOString() }));
      if (newPassword) newPassword.value = '';
      if (confirmPassword) confirmPassword.value = '';
      if (window.skilliantLogActivity) window.skilliantLogActivity('Password reset', 'Contractor account password was reset successfully.');
