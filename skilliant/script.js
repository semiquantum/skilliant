...
function initOtpInputs() {
  ...
  // Retrieve targeted email from URL query param or sessionStorage
  const urlParams = new URLSearchParams(window.location.search);
  const targetEmail = urlParams.get('email') || sessionStorage.getItem('skilliant_verify_email') || '';

  const emailDisplay = document.getElementById('verificationEmailTarget');
  if (emailDisplay && targetEmail) {
    emailDisplay.textContent = targetEmail;
  }

  const changeLink = document.getElementById('changeEmailLink');
  if (changeLink && targetEmail) {
    changeLink.href = `register.html?email=${encodeURIComponent(targetEmail)}`;
  }
  ...
  otpBoxes.forEach((box, index) => {
    box.addEventListener('input', (e) => {
      ...
      // Remove any previous error highlight
  ...
  if (timerEl && resendBtn) {
    ...
    resendBtn.addEventListener('click', async (e) => {
      ...
      resendBtn.style.opacity = '0.5';
      resendBtn.style.pointerEvents = 'none';
      showToast(`Requesting new verification code for ${targetEmail}...`, 'info');
      ...
      try {
        ...
        if (result.success) {
          startTimer();
          showToast(result.message || `New verification code sent to ${targetEmail}!`, 'success', 6000);
        } else {
          resendBtn.removeAttribute('disabled');
          resendBtn.style.opacity = '1';
          resendBtn.style.pointerEvents = 'auto';
        ...
        resendBtn.style.opacity = '1';
        resendBtn.style.pointerEvents = 'auto';
        showToast('Network error while requesting new verification code.', 'error');
      }
    });
  }
}
...
function initForms() {
  ...
  if (registerForm) {
    ...
    if (registerEmailInput && prefillEmail) {
      registerEmailInput.value = prefillEmail;
    }
    ...
    registerForm.addEventListener('submit', (e) => {
      ...
      if (!validateEmail(emailVal)) {
        setFieldValidation(email, false, 'Please enter a valid, well-formed email address (e.g. name@domain.com)');
        valid = false;
      } else {
        setFieldValidation(email, true);
      }
      ...
      if (valid) {
        ...
        const userEmail = emailVal.toLowerCase();
        ...
        .then(result => {
          ...
          if (result.success) {
            sessionStorage.setItem('skilliant_verify_email', userEmail);
            showToast(result.message || `Verification code sent to ${userEmail}!`, 'success', 5000);
            setTimeout(() => {
              window.location.href = `email-verification.html?email=${encodeURIComponent(userEmail)}`;
            }, 600);
          } else {
            setFieldValidation(email, false, result.error || 'Failed to dispatch verification code.');
        ...
        .catch(err => {
          submitBtn.classList.remove('loading');
          submitBtn.removeAttribute('disabled');
          setFieldValidation(email, false, 'Unable to connect to verification server. Please try again.');
          showToast('Network error connecting to verification service.', 'error');
        });
      }
    });
  }
  ...
  if (otpForm) {
    otpForm.addEventListener('submit', async (e) => {
      ...
      // Get targeted email from URL or session
      const urlParams = new URLSearchParams(window.location.search);
      const targetEmail = urlParams.get('email') || sessionStorage.getItem('skilliant_verify_email') || '';

      if (!targetEmail) {
        showToast('No email address provided for verification. Please register first.', 'error');
      ...
      if (otpCode.length < 6) {
        if (errorAlert) {
          errorAlert.textContent = 'Please enter all 6 digits of the verification code.';
      ...
      try {
        ...
        if (result.success && result.status === 'approved') {
          showToast('Email verified successfully! Setting up your account...', 'success');
          sessionStorage.removeItem('skilliant_verify_email');
          setTimeout(() => {
            window.location.href = 'success.html';
          }, 800);
        } else {
          ...
          const msg = result.error || 'Incorrect or expired verification code. Please check and try again.';
          if (errorAlert) {
        ...
        submitBtn.classList.remove('loading');
        submitBtn.removeAttribute('disabled');
        const errMsg = 'Network error during verification. Please check your connection and try again.';
        if (errorAlert) {
